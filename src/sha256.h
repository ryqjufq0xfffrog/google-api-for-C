char* txt2sha256(char* input, int length) {
  const int bufLength = ((length + 8 + 1) / 64 + 1) * 64;
  ...
  memcpy(buffer + bufLength - length, input, length);
  // Padding
  buffer[0] = (char) 0x80;
  for(int i = 1; i < bufLength - length; i++) {
    buffer[i] = 0;
  }
